    ...
    private void EvaluateChunksAroundPlayer()
    {
        ...
        for (int x = -chunksInRadius; x <= chunksInRadius; x++)
        {
            for (int y = -chunksInRadius; y <= chunksInRadius; y++)
            {
                ...
                // Never spawn within the clear radius — mark as checked so it
                // stays blocked even after the player moves away from it
                if (Vector2.Distance(playerPos, chunkCentre) < clearRadius)
                {
                    checkedChunks[chunkKey] = false;
                    continue;
                }
